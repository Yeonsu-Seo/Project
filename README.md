...
<p>저의 취미는 스도쿠로 어릴 때부터 매일 스도쿠 문제를 풉니다. 종이책으로 풀 때도 있지만 보통 어플에서 스도쿠 문제를 푸는데, 요즘 어플에 광고가 많아지면서 제가 직접 스도쿠 게임을 하는 프로그램을 만들어서 광고없이 즐기면 좋을 것 같다는 생각을 하면서 스도쿠를 프로젝트 주제로 정하게 되었습니다.</p>
<br/>

## $\bf{\color{#6580DD}2. 프로그램 기능}$
### 1. 난이도 선택
...
## $\bf{\color{#6580DD}5. 더 나아가...}$
<p>프로젝트를 하면서 데이터(스도쿠 문제)를 웹에서 가져올 수 있다면 더 많은 데이터를 사용할 수 있어서 BeautifulSoup, Requests를 사용하여 Web scraping을 할 수 있으면 더 좋았을 것 같습니다. 하지만 스도쿠 문제들 크롤링할 수 있는 형태로 모아놓은 사이트를 찾지 못하여 한정된 데이터를 사용하는 방식으로 진행하였습니다. 그래서 후속 프로젝트로 스도쿠 문제 판을 생성하는 프로그램을 만들면 좋을 것 같습니다.</p>
